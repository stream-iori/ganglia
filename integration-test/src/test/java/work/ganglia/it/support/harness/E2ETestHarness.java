...
public class E2ETestHarness {
  ...
  public Future<String> runScenario(TestScenario scenario) {
    ...
    return ganglia
        .agentLoop()
        .run(scenario.userInput(), context)
  ...
  private Future<String> handleInteractiveSteps(
      TestScenario scenario, SessionContext context, String currentOutput, int stepIndex) {
    ...
    if (step.expectedInterruptPrompt() != null) {
      assertTrue(
          currentOutput.contains(step.expectedInterruptPrompt()),
    ...
    for (TestScenario.Expectation exp : scenario.expectations()) {
      switch (exp.type()) {
        case "OUTPUT_CONTAINS" ->
            assertTrue(result.contains(exp.value()), "Output should contain: " + exp.value());
        case "FILE_EXISTS" ->
            assertTrue(Files.exists(Path.of(exp.value())), "File should exist: " + exp.value());
        case "MEMORY_CONTAINS" -> {
          ...
          try {
            ...
            assertTrue(memory.contains(exp.value()), "Memory should contain: " + exp.value());
          } catch (Exception e) {
