...
public class KnowledgeAcquisitionE2EIT {
  ...
  void setUp(Vertx vertx, VertxTestContext testContext) {
    ...
    httpServer
        ...
        .compose(
            v ->
                harness.setup(
                    new JsonObject().put("agent", new JsonObject().put("projectRoot", "."))))
  ...
  void testWebFetchAndRemember(Vertx vertx, VertxTestContext testContext) {
    ToolCall fetchCall =
        new ToolCall("c1", "web_fetch", Map.of("url", "http://localhost:" + port + "/readme.md"));
    ToolCall rememberCall =
        new ToolCall("c2", "remember", Map.of("fact", "Convention: Use Java 17 records."));

    TestScenario scenario =
        new TestScenario(
            "scenario1",
            "Knowledge Acquisition",
            "Fetch the content from http://localhost:"
                + port
                + "/readme.md and remember the project conventions.",
            List.of(
                new ModelResponse(
                    "Fetching conventions...", List.of(fetchCall), new TokenUsage(1, 1)),
                new ModelResponse(
