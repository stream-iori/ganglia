...
public class MemoryRefactorIT {
  ...
  void testObservationCompressionAndRecall(Vertx vertx, VertxTestContext testContext) {
    String longOutput =
        "This is a very long output that should be compressed. ".repeat(200); // Definitely > 4000
    ...
    when(mockModel.chatStream(any(), any()))
        .thenReturn(
            Future.succeededFuture(
                new ModelResponse("", List.of(longToolCall), new TokenUsage(1, 1))))
        .thenAnswer(
            invocation -> {
              ChatRequest req = invocation.getArgument(0);
              String lastMessage = req.messages().get(req.messages().size() - 1).content();
              assertTrue(
                  lastMessage.contains("compressed"), "Last message should mention compression");
              ...
              return Future.succeededFuture(
                  new ModelResponse("", List.of(recallCall), new TokenUsage(1, 1)));
            })
        .thenAnswer(
            invocation -> {
              ChatRequest req = invocation.getArgument(0);
              ...
              return Future.succeededFuture(
                  new ModelResponse(
                      "Final Answer with full content info.",
    ...
    // Since we use the real ToolExecutor but mock the LLM, we should probably mock the tool output
    // if possible,
    // but StandardToolTask calls toolExecutor.execute().
    // For simplicity, let's just use a fake tool that returns long output by overriding the
    // ToolExecutor if we could,
