...
public class SubAgentGraphIT {
    ...
    void testSequentialGraphExecution(Vertx vertx, VertxTestContext testContext) {
        ...
        ToolCall graphCall = new ToolCall("c1", "propose_task_graph", Map.of(
            "nodes", List.of(
                Map.of("id", "n1", "task", "Task 1", "persona", "GENERAL"),
                Map.of("id", "n2", "task", "Task 2", "persona", "GENERAL", "dependencies", List.of("n1"))
            ),
            "approved", true
        ));

        when(mockModel.chatStream(any(ChatRequest.class), any()))
            .thenReturn(Future.succeededFuture(new ModelResponse("Executing graph...", List.of(graphCall), new TokenUsage(10, 10))))
            .thenReturn(Future.succeededFuture(new ModelResponse("Task 1 Done", Collections.emptyList(), new TokenUsage(5, 5))))
            .thenReturn(Future.succeededFuture(new ModelResponse("Task 2 Done", Collections.emptyList(), new TokenUsage(5, 5))))
            .thenReturn(Future.succeededFuture(new ModelResponse("Graph completed successfully.", Collections.emptyList(), new TokenUsage(10, 10))));

        SessionContext context = ganglia.sessionManager().createSession(UUID.randomUUID().toString());

        ganglia.agentLoop().run("Run a sequence of tasks", context)
