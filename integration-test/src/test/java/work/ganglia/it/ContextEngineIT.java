...
public class ContextEngineIT {
  ...
  void testDynamicInstructionFileInPrompt(Vertx vertx, VertxTestContext testContext) {
    ...
    JsonObject configOverride =
        new JsonObject().put("agent", new JsonObject().put("instructionFile", customFile));
                  ...
                  testContext.verify(
                      () -> {
                        assertTrue(
                            ...
                            "Prompt should contain the custom instruction filename: " + customFile);
                        testContext.completeNow();
                      });
                }));
  }
