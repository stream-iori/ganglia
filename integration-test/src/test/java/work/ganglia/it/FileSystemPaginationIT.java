...
public class FileSystemPaginationIT {
  ...
  void setUp(Vertx vertx, VertxTestContext testContext) {
    ...
    ModelResponse response2 =
        new ModelResponse(
            "I have read the first 5 lines.", Collections.emptyList(), new TokenUsage(10, 10));
        ...
        .onComplete(
            testContext.succeeding(
                (Ganglia g) -> {
                  this.ganglia = g;
                  this.baseContext =
                      ganglia.sessionManager().createSession(UUID.randomUUID().toString());
  ...
  void testPaginatedReadWorkflow(Vertx vertx, VertxTestContext testContext) {
    ...
    ganglia
        .agentLoop()
        .run("Read the first 5 lines of large_file.txt", baseContext)
        ...
        .onComplete(
            testContext.succeeding(
                resultContext -> {
                  testContext.verify(
                      () -> {
                        // Check if the output of the tool execution in context contains pagination
                        // info
                        ...
                        assertTrue(
                            foundToolOutput,
                            "Pagination metadata not found in history: " + resultContext.history());
                        testContext.completeNow();
                      });
                }));
  }
