...
class MemoryServiceTest {
  ...
  void testHandleReflectEvent(Vertx vertx, VertxTestContext testContext) {
    ...
    ModelResponse reflectionResponse =
        new ModelResponse(accomplishment, Collections.emptyList(), new TokenUsage(10, 10));
    modelGateway.addResponse(reflectionResponse);

    // Publish event to EventBus
    MemoryEvent event =
        new MemoryEvent(MemoryEvent.EventType.TURN_COMPLETED, sessionId, goal, turn);

    vertx.eventBus().publish(Constants.ADDRESS_MEMORY_EVENT, JsonObject.mapFrom(event));
                            ...
                            vertx
                                ...
                                .onSuccess(
                                    buffer -> {
                                      ...
                                      if (content.contains(accomplishment)
