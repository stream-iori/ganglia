...
- **Sub-Agent Orchestration:** Delegate specialized sub-tasks to isolated child agents, with DAG-based parallel execution, mission context propagation, selective input mapping, and cross-cycle fact sharing via the Blackboard.
...
| `ganglia-coding-webui`      | React 18 multi-page frontend (Coding UI + Trace Studio)                   |
...
| `GraphExecutor` / `DefaultGraphExecutor`       | DAG-based orchestration with mission context and `inputMapping` wiring   |
| `Blackboard` / `InMemoryBlackboard`            | Append-only fact store for cross-cycle state sharing between Managers    |
...
| `AgentSignal`                  | Abort signal with `AbortReason` (USER_CANCELLED, MISSION_SUPERSEDED, BUDGET_EXCEEDED, STALL_DETECTED) |
| `ToolDefinition`               | Tool name, description, JSON schema, interrupt flag               |
| `ToolCall` / `AgentTaskResult` | Tool invocation and its result                                    |
| `Fact` / `FactStatus`          | Immutable versioned fact record with lifecycle (ACTIVE/SUPERSEDED/ARCHIVED) |
| `ObservationType`              | Full enum of lifecycle event types (incl. FACT_PUBLISHED/SUPERSEDED/ARCHIVED) |
...
## 🌐 ganglia-coding-web Capabilities
