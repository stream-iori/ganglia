...
class InterceptorPipelineTest {
    ...
    void setUp() {
        ...
        context = new SessionContext("test-id", Collections.emptyList(), null, Collections.emptyMap(), Collections.emptyList(), null);
    }

    @Test
    void testPreTurnChaining(VertxTestContext testContext) {
        AtomicInteger counter = new AtomicInteger(0);
        pipeline.addInterceptor(new AgentInterceptor() {
            @Override
        ...
        pipeline.executePreTurn(context, "hello").onComplete(testContext.succeeding(res -> {
            testContext.verify(() -> {
    ...
    void testPostTurnErrorIsCaught(VertxTestContext testContext) {
        ...
        pipeline.executePostTurn(context, Message.assistant("done")).onComplete(testContext.succeeding(res -> {
            testContext.verify(() -> {
    ...
    void testPreToolExecuteFailureInterruptsChain(VertxTestContext testContext) {
        ...
        pipeline.executePreToolExecute(call, context).onComplete(testContext.failing(err -> {
            testContext.verify(() -> {
