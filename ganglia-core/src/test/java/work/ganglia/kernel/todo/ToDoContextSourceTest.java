...
class ToDoContextSourceTest {
    ...
    void testLayeredToDoFragments(VertxTestContext testContext) {
        ...
        SessionContext context = new SessionContext("test", List.of(), null, Map.of("todo_list", toDoList), List.of(), null);

        source.getFragments(context).onComplete(ar -> {
            if (ar.failed()) {
            ...
            ContextFragment summary = fragments.stream().filter(f -> f.name().contains("Summary")).findFirst().orElseThrow();
            ...
            ContextFragment details = fragments.stream().filter(f -> f.name().contains("Details")).findFirst().orElseThrow();
