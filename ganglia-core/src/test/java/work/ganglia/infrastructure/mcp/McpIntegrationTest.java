...
public class McpIntegrationTest {
    ...
    void setUp(Vertx vertx, VertxTestContext testContext) {
        ...
        server.start().onComplete(testContext.succeeding(v -> {
            StreamMcpTransport transport = new StreamMcpTransport(vertx, new WebSocketConnectOptions().setHost("localhost").setPort(port).setURI("/"));
    ...
    void testInitializeAndListTools(Vertx vertx, VertxTestContext testContext) {
        McpInitializeRequest.Implementation clientInfo = new McpInitializeRequest.Implementation("TestClient", "1.0");
        McpInitializeRequest initReq = new McpInitializeRequest("2025-11-25", new HashMap<>(), clientInfo);

        client.initialize(initReq)
            .compose(initRes -> {
                assertEquals("2025-11-25", initRes.protocolVersion());
                return client.listTools();
            })
            .onComplete(testContext.succeeding(listRes -> {
                testContext.verify(() -> {
