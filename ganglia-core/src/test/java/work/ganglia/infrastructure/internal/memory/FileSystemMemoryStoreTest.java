...
class FileSystemMemoryStoreTest {
    ...
    void testStoreAndRecall(Vertx vertx, VertxTestContext testContext) {
        ...
        store.store(entry)
            ...
            .onComplete(testContext.succeeding(recalled -> {
                testContext.verify(() -> {
    ...
    void testGetRecentIndex(Vertx vertx, VertxTestContext testContext) {
        ...
        store.store(e1)
            ...
            .onComplete(testContext.succeeding(index -> {
                testContext.verify(() -> {
    ...
    void testSearchByKeyword(Vertx vertx, VertxTestContext testContext) {
        ...
        store.store(e1)
            ...
            .onComplete(testContext.succeeding(results -> {
                testContext.verify(() -> {
    ...
    private MemoryEntry createEntry(String id, String title, Instant timestamp) {
        return new MemoryEntry(
            id, title, "summary", "content", MemoryCategory.UNKNOWN,
            Collections.emptyList(), timestamp, Collections.emptyList()
        );
    }
}