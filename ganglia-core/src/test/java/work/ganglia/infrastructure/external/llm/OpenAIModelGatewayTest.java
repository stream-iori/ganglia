...
class OpenAIModelGatewayTest {
    ...
    void setUp(VertxTestContext testContext) {
        ...
        gateway = new OpenAIModelGateway(vertx, WebClient.create(vertx), "test-key", "http://localhost:" + port);
                    ...
                    if (requestPayload.getBoolean("stream", false)) {
                        ...
                        JsonObject response = new JsonObject()
                            .put("choices", new JsonArray().add(
                                new JsonObject().put("message", new JsonObject().put("content", "Hello OpenAI!"))
                            ))
                            .put("usage", new JsonObject().put("prompt_tokens", 5).put("completion_tokens", 10));
