...
class AnthropicModelGatewayTest {
    ...
    void setUp(VertxTestContext testContext) {
        ...
        gateway = new AnthropicModelGateway(vertx, WebClient.create(vertx), "test-key", "http://localhost:" + port);
                ...
                req.bodyHandler(body -> {
                    ...
                    if (requestPayload.getBoolean("stream", false)) {
                        ...
                        JsonObject response = new JsonObject()
                            .put("content", new JsonArray().add(new JsonObject().put("type", "text").put("text", "Hello World!")))
                            .put("usage", new JsonObject().put("input_tokens", 10).put("output_tokens", 15));
                        req.response().putHeader("Content-Type", "application/json").end(response.encode());
                    }
                });
            } else {
                req.response().setStatusCode(404).end();
            }
        }).listen(port).onComplete(testContext.succeeding(s -> testContext.completeNow()));
    }
