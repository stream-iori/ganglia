...
public class ToDoContextSource implements ContextSource {
    ...
    public Future<List<ContextFragment>> getFragments(SessionContext sessionContext) {
        ...
        if (toDoList == null || toDoList.isEmpty()) {
            String emptyNote = "No active plan. You should create one using todo_add if the task is complex.";
            return Future.succeededFuture(List.of(
                ContextFragment.mandatory("Plan Strategy", emptyNote, 12)
            ));
        }

        // 1. Mandatory Core Plan (High priority, summarized, ensures continuity)
        String corePlan = toDoList.toSummarizedString() + 
        ...
        return Future.succeededFuture(List.of(
            ...
            ContextFragment.prunable("Plan Details (Extended)", detailedPlan, ContextFragment.PRIORITY_PLAN)
        ));
    }
}
