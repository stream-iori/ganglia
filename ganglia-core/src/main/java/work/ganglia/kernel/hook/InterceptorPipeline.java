...
/**
 * Manages the execution of registered AgentInterceptors.
 * It uses Vert.x Future.compose() to run hooks sequentially.
 */
public class InterceptorPipeline {
    ...
    public Future<ToolInvokeResult> executePostToolExecute(ToolCall call, ToolInvokeResult result, SessionContext context) {
    ...
    public Future<Void> executePostTurn(SessionContext context, Message finalResponse) {
        ...
        for (AgentInterceptor hook : interceptors) {
            f = f.compose(v -> hook.postTurn(context, finalResponse)
                .recover(err -> {
                    // Fire-and-forget logic: Catch errors here so one failing hook 
                    // doesn't stop subsequent postTurn hooks.
                    log.warn("Interceptor {} failed during postTurn, but continuing", hook.getClass().getSimpleName(), err);
