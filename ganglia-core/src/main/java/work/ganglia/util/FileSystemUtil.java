...
public class FileSystemUtil {
  ...
  /**
   * Ensures that a file exists, creating it with default content if it does not. Also ensures that
   ...
   */
  public static Future<Void> ensureFileWithDefault(
      Vertx vertx, String filePath, Buffer defaultContent) {
    return vertx
        .fileSystem()
        .exists(filePath)
        .compose(
            exists -> {
              ...
              Future<Void> dirFuture =
                  parent != null
                      ? ensureDirectoryExists(vertx, parent.toString())
                      : Future.succeededFuture();

              return dirFuture.compose(
                  v -> {
   ...
   */
  public static Future<Void> ensureFileWithDefault(
      Vertx vertx, String filePath, String defaultContent) {
