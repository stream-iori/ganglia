...
public class FileSystemMemoryStore implements MemoryStore {
    ...
    public FileSystemMemoryStore(Vertx vertx, String basePath) {
        this.vertx = vertx;
        this.memoryDir = Paths.get(basePath).resolve(".ganglia/memory/entries").toAbsolutePath().normalize();
    ...
    private void loadIndex() {
        ...
        for (String file : files) {
            try {
                ...
                index.add(new MemoryIndexItem(entry.id(), entry.title(), entry.category(), entry.timestamp()));
            } catch (Exception e) {
    ...
    public Future<Void> store(MemoryEntry entry) {
        ...
        return vertx.fileSystem().writeFile(filePath, io.vertx.core.buffer.Buffer.buffer(json))
            .onSuccess(v -> {
    ...
    private boolean matchesQuery(MemoryEntry entry, MemoryQuery query) {
        ...
        if (query.tags() != null && !query.tags().isEmpty()) {
            boolean hasAllTags = query.tags().stream().allMatch(queryTag -> 
                entry.tags() != null && entry.tags().stream().anyMatch(entryTag -> 
                    entryTag.name().equals(queryTag.name()) && entryTag.value().equals(queryTag.value())
                )
            );
            if (!hasAllTags) {
                return false;
        ...
        if (query.keyword() != null && !query.keyword().isBlank()) {
            ...
            boolean matchSummary = entry.summary() != null && entry.summary().toLowerCase().contains(keyword);
            boolean matchContent = entry.fullContent() != null && entry.fullContent().toLowerCase().contains(keyword);
    ...
    public Future<MemoryEntry> recall(String id) {
        ...
        return vertx.fileSystem().readFile(filePath)
            ...
            .recover(err -> Future.failedFuture(new RuntimeException("Failed to recall memory " + id, err)));
    }
}