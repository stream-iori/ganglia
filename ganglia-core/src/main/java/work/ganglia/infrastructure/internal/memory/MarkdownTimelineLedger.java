...
public class MarkdownTimelineLedger implements TimelineLedger {
    ...
    public MarkdownTimelineLedger(Vertx vertx, String basePath) {
        ...
        if (!vertx.fileSystem().existsBlocking(timelineFile.toString())) {
            ...
            vertx.fileSystem().writeFileBlocking(timelineFile.toString(), io.vertx.core.buffer.Buffer.buffer(header));
        }
    }
    ...
    public Future<Void> appendEvent(TimelineEvent event) {
        try {
            ...
            markdown.append("### [").append(timestampStr).append("] ").append(event.category()).append("\n");
            ...
            io.vertx.core.file.OpenOptions options = new io.vertx.core.file.OpenOptions().setAppend(true).setCreate(true);
            return vertx.fileSystem().open(timelineFile.toString(), options)
                .compose(file -> file.write(io.vertx.core.buffer.Buffer.buffer(markdown.toString()))
