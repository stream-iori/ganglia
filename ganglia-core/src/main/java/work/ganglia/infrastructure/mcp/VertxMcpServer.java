...
/**
 * A lightweight MCP Server implementation strictly for integration testing.
 * Uses WebSockets as the streamable transport.
 */
                ...
                if (message.containsKey("method")) {
                    ...
                    if ("initialize".equals(method)) {
                        response.put("result", new JsonObject()
                            ...
                            .put("serverInfo", new JsonObject().put("name", "TestServer").put("version", "1.0"))
                        );
                        ...
                        tools.add(new JsonObject()
                            ...
                            .put("inputSchema", new JsonObject().put("type", "object").put("properties", new JsonObject().put("text", new JsonObject().put("type", "string"))))
                        );
                        ...
                        if ("echo".equals(name)) {
                            ...
                            JsonArray content = new JsonArray().add(new JsonObject().put("type", "text").put("text", "Echo: " + textArg));
                            response.put("result", new JsonObject().put("content", content).put("isError", false));
                        } else {
                            response.put("error", new JsonObject().put("code", -32601).put("message", "Tool not found"));
                        }
