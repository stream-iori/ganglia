...
public class McpConfigManager {
    ...
    public static Future<McpRegistry> loadMcpToolSets(Vertx vertx, String projectRoot) {
        String mcpConfigPath = Paths.get(projectRoot, Constants.DEFAULT_GANGLIA_DIR, FILE_MCP_JSON).toString();

        return FileSystemUtil.ensureFileWithDefault(vertx, mcpConfigPath, io.vertx.core.buffer.Buffer.buffer(DEFAULT_MCP_CONFIG))
    ...
    private static Future<McpLoadResult> loadServer(Vertx vertx, String serverName, JsonObject config) {
        ...
        return transport.connect()
            .compose(v -> {
                McpInitializeRequest.Implementation clientInfo = new McpInitializeRequest.Implementation("Ganglia", "1.0");
                McpInitializeRequest initReq = new McpInitializeRequest("2025-11-25", new HashMap<>(), clientInfo);
                return client.initialize(initReq);
            })
            .compose(res -> McpToolSet.create(client))
            .map(ts -> {
                log.info("Successfully loaded MCP server '{}' with {} tools", serverName, ts.getDefinitions().size());
