...
public class TaskPanelRendererTest {
    ...
    void testRenderHeaderShowsActiveTask() {
        ToDoList plan = new ToDoList(List.of(
                new ToDoItem("1", "Implement feature X", TaskStatus.IN_PROGRESS)
        ));
        renderer.updatePlan(plan);
    ...
    void testRenderWithNoActiveTaskShowsGenericHeader() {
        ToDoList plan = new ToDoList(List.of(
                new ToDoItem("1", "Done task", TaskStatus.DONE)
        ));
        renderer.updatePlan(plan);
    ...
    void testUpdatePlanFromDataWithMap() {
        ...
        Map<String, Object> planMap = Map.of(
                "items", List.of(
                        Map.of("id", "1", "description", "Test task", "status", "TODO")
                )
        );
    ...
    void testUpdatePlanFromDataWithToDoList() {
        ToDoList plan = new ToDoList(List.of(
                new ToDoItem("1", "Direct plan", TaskStatus.TODO)
        ));
