...
/**
 * Renders the task panel in the fixed bottom area of the terminal.
 * Shows active tasks with elapsed time and completed/pending task tree.
 */
public class TaskPanelRenderer {
    ...
    /**
     * Returns the number of rows needed to render the task panel.
    ...
    /**
     * Deserializes a ToDoList from EventBus data map.
     * EventBus serializes records as LinkedHashMap; this reconstructs the proper type.
     */
    ...
    public void renderAt(PrintWriter writer, int startRow, int cols, int maxRows) {
        ...
        ToDoItem activeTask = items.stream()
                .filter(i -> i.status() == TaskStatus.IN_PROGRESS)
                .findFirst()
                .orElse(null);
        ...
        if (activeTask != null) {
            ...
            String header = "\033[1m* " + truncate(activeTask.description(), cols - elapsed.length() - 5)
                    + " " + elapsed + "\033[0m";
            writer.print(header);
        } else {
