...
/**
 * Full-screen alternate-buffer viewer for tool execution details.
 * Supports scrolling with arrow keys, PgUp/PgDn, Home/End.
 * Press q or ESC to return to the main screen.
 */
public class DetailView {
    ...
    /**
     * Shows the tool card detail in an alternate screen buffer.
                    ...
                    case '\033' -> { // ESC or escape sequence
                        ...
                        if (next == -1 || next == '\033') {
                            ...
                            switch (code) {
                                ...
                                case '5' -> { readChar(); scrollOffset = Math.max(0, scrollOffset - viewportRows); }
                                case '6' -> { readChar(); scrollOffset = Math.min(maxScroll, scrollOffset + viewportRows); }
    ...
    /**
     * Builds the content lines displayed in the scrollable area.
