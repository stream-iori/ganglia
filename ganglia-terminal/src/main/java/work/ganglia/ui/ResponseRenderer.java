...
/**
 * Renders assistant responses with dot-prefixed lines and supports
 * collapse/expand toggling (both append and in-place modes).
 */
public class ResponseRenderer {
     ...
     *
     * <p>When the old (expanded) content was taller than the terminal, the cursor
     * cannot move above row 1. We cap the upward movement to the visible area and
     * only clear the stale lines that are actually on screen. We must NOT use
     * {@code \033[J} (clear-to-end-of-screen) because the StatusBar lives below
     * the scroll region and would be wiped.
     */
    ...
    private List<String> buildResponseLines(String content, boolean full) {
        ...
        for (int i = 0; i < displayCount; i++) {
            ...
            result.add(new AttributedStringBuilder()
                    .style(style)
                    .append(dot)
