...
public class ToolResultCompactor {
  ...
  /**
   * Tools whose output must never be compacted or compressed. RealityAnchor validation output is
   * tagged as uncompressible to prevent interpretation bias during context optimization.
   */
  public static final Set<String> UNCOMPRESSIBLE_TOOLS = Set.of("reality_anchor");
  ...
  /** Collects tool results from a single turn, skipping uncompressible tools. */
  private void collectFromTurn(
    ...
    for (Message msg : turn.intermediateSteps()) {
      if (msg.role() == Role.TOOL
          ...
          && !UNCOMPRESSIBLE_TOOLS.contains(msg.toolObservation().toolName().toLowerCase())
          && (toolFilter == null
