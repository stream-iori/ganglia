...
public class ContextSlimmer {
  ...
  /**
   * Clears old tool result content when the server-side prompt cache has likely expired.
   *
   * <p>This is a proactive optimization: after a gap in activity (typically 5+ minutes), the LLM
   * provider's prompt cache may have expired. Old tool results that were cached are no longer
   * benefiting from cache, so we replace their content with a placeholder to save tokens while
   * preserving the conversation structure.
   *
   * @param context the session context
  ...
  public SessionContext slimOldToolResults(SessionContext context, long maxAgeMs) {
    ...
    for (Turn turn : turns) {
      if (turn.timestamp() < cutoffTime) {
        Turn slimmedTurn = clearToolResultsFromTurn(turn);
        if (slimmedTurn != turn) {
          anyChanged = true;
        }
        newTurns.add(slimmedTurn);
    ...
    logger.info(
        "Cleared old tool results from {} turns older than {} ms", newTurns.size(), maxAgeMs);
    return context.withPreviousTurns(newTurns);
  }

  /**
   * Clears tool result content from a turn, replacing with placeholders.
   *
   * @return the slimmed turn, or the original turn if no changes were made
   */
  private Turn clearToolResultsFromTurn(Turn turn) {
    List<Message> newSteps = new ArrayList<>();
    boolean anyChanged = false;

    for (Message msg : turn.intermediateSteps()) {
      if (msg.role() == Role.TOOL && msg.content() != null && !msg.content().isEmpty()) {
        // Replace tool result with placeholder
        String toolName = msg.toolObservation() != null ? msg.toolObservation().toolName() : "tool";
        String placeholder = "[Old tool result cleared - cache expired: " + toolName + "]";
        msg = Message.tool(msg.toolObservation().toolCallId(), toolName, placeholder);
        anyChanged = true;
      }
      newSteps.add(msg);
    }
