...
class LLMObservationCompressorTest {
  ...
  void testCompress(VertxTestContext testContext) {
    ...
    when(modelGateway.chat(any()))
        .thenReturn(
            Future.succeededFuture(
                new ModelResponse(
                    compressedSummary, Collections.emptyList(), new TokenUsage(10, 5))));

    CompressionContext context = new CompressionContext("ls", "find files", 50);

    compressor
        .compress(rawOutput, context)
        .onComplete(
  ...
  void testNoCompressionNeeded(VertxTestContext testContext) {
    ...
    compressor
        .compress(rawOutput, new CompressionContext("tool", "task", 50))
        .onComplete(
            testContext.succeeding(
                result -> {
                  testContext.verify(
