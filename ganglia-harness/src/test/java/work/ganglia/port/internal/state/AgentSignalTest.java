...
class AgentSignalTest {
  ...
  @Test
  void abort_noArg_defaultsToUserCancelled() {
    AgentSignal signal = new AgentSignal();
    signal.abort();

    assertEquals(AgentSignal.AbortReason.USER_CANCELLED, signal.getReason());
  }

  @Test
  void abort_withReason_storesReason() {
    AgentSignal signal = new AgentSignal();
    signal.abort(AgentSignal.AbortReason.MISSION_SUPERSEDED);

    assertTrue(signal.isAborted());
    assertEquals(AgentSignal.AbortReason.MISSION_SUPERSEDED, signal.getReason());
  }

  @Test
  void abort_withBudgetExceeded() {
    AgentSignal signal = new AgentSignal();
    signal.abort(AgentSignal.AbortReason.BUDGET_EXCEEDED);

    assertEquals(AgentSignal.AbortReason.BUDGET_EXCEEDED, signal.getReason());
  }

  @Test
  void abort_withStallDetected() {
    AgentSignal signal = new AgentSignal();
    signal.abort(AgentSignal.AbortReason.STALL_DETECTED);

    assertEquals(AgentSignal.AbortReason.STALL_DETECTED, signal.getReason());
  }

  @Test
  void getReason_beforeAbort_returnsNull() {
    AgentSignal signal = new AgentSignal();
    assertNull(signal.getReason());
  }

  @Test
  void abort_idempotent_firstReasonWins() {
    AgentSignal signal = new AgentSignal();
    signal.abort(AgentSignal.AbortReason.BUDGET_EXCEEDED);
    signal.abort(AgentSignal.AbortReason.STALL_DETECTED);

    assertEquals(
        AgentSignal.AbortReason.BUDGET_EXCEEDED,
        signal.getReason(),
        "First abort reason should win");
  }

  @Test
  void abort_withReason_callbacksStillFire() {
    AgentSignal signal = new AgentSignal();
