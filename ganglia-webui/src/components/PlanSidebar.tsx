              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between gap-2 mb-1">
                  <span className={cn(
                    ...
                    item.status === 'DONE' ? "text-emerald-500/70" : item.status === 'IN_PROGRESS' ? "text-blue-400" : "text-slate-500"
                  )}>
                ...
                {item.result && (
                  <div className="mt-3 p-3 bg-black/40 rounded-lg border border-white/5 text-[11px] text-slate-400 font-mono leading-normal">
                    <div className="text-[9px] uppercase tracking-tighter text-slate-600 mb-1 font-bold">Execution Summary</div>
                    {item.result}
                  </div>
                )}
              </div>
