...
const TaskReviewCard: React.FC = () => {
  ...
  const handleMemorySave = () => {
    eventBusService.send('START', {
      prompt:
        'Please summarize the learnings from this task and save them to .ganglia/memory/MEMORY.md.',
    });
  };
  ...
  return (
    <div className="my-6 bg-slate-900 border border-emerald-500/30 rounded-xl overflow-hidden shadow-[0_0_30px_rgba(16,185,129,0.1)]">
      <div className="bg-emerald-500/10 px-6 py-4 border-b border-emerald-500/20 flex items-center gap-3">
        ...
        <div>
          <h3 className="text-sm font-bold text-emerald-500 uppercase tracking-tight">
            Task Completed
          </h3>
          <p className="text-xs text-slate-400 mt-0.5">
            The agent has finished the requested task.
          </p>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          >
            ...
            <span className="text-xs text-slate-500 mt-1">
              Open the global diff inspector to verify all file modifications.
            </span>
          </button>
          ...
          >
            ...
            <span className="text-xs text-slate-500 mt-1">
              Instruct the agent to document learnings from this session.
            </span>
          </button>
        </div>
      </div>
