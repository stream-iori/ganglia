...
const Sidebar: React.FC = () => {
  ...
  const newSession = () => {
    if (window.confirm('Start a new session? Current history will be hidden until you switch back.')) {
          ...
          <div className="mt-2 text-[10px] flex items-center gap-2 uppercase tracking-tighter font-bold">
            <span
              className={cn(
                ...
                systemStore.status === 'CONNECTED' && 'bg-emerald-500 shadow-[0_0_5px_rgba(16,185,129,0.5)]',
                systemStore.status === 'RECONNECTING' && 'bg-amber-500',
                systemStore.status === 'DISCONNECTED' && 'bg-rose-500'
              )}
            ></span>
            <span className={systemStore.status === 'CONNECTED' ? 'text-slate-200' : 'text-slate-500'}>
              {systemStore.status}
            </span>
          </div>
        </div>
      ...
      <div className="flex border-b border-slate-800 px-2 bg-slate-950/30">
        <button
          ...
          className={cn(
            ...
            activeTab === 'FILES' ? 'border-emerald-500 text-emerald-500' : 'border-transparent text-slate-500 hover:text-slate-300'
          )}
        >
          Files
        </button>
        <button
          ...
          className={cn(
            'flex-1 py-2 text-[10px] font-bold uppercase tracking-widest transition-colors border-b-2',
            activeTab === 'SESSIONS' ? 'border-emerald-500 text-emerald-500' : 'border-transparent text-slate-500 hover:text-slate-300'
          )}
        >
          Sessions
        </button>
          ...
          <div className="p-4">
            <div className="mb-6">
              <div className="flex items-center justify-between mb-3">
                <h2 className="text-[10px] font-bold uppercase tracking-widest text-slate-500">Workspace</h2>
                <button
              ...
              {logStore.fileTree ? (
                ...
                <div className="text-[10px] opacity-40 italic px-2">Loading project structure...</div>
              )}
            </div>
          </div>
        ) : (
          <div className="p-4">
            <h2 className="text-[10px] font-bold uppercase tracking-widest text-slate-500 mb-4">Recent Sessions</h2>
              ...
              {systemStore.sessionHistory.map((id) => (
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    {systemStore.sessionId === id && (
                      <span className="text-[8px] bg-emerald-500 text-black px-1 rounded font-bold">ACTIVE</span>
                    )}
                  </div>
                  <span className="text-[9px] text-slate-600 group-hover:text-slate-500">History synced from backend</span>
                </button>
              ))}
            </div>
          </div>
