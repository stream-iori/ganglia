...
import type { 
  ...
  StartParams, RespondAskParams, ReadFileParams, SyncParams, ListFilesParams,
  InitConfigData
} from '../types'
    ...
    window.WebSocket = class MockWebSocket {
      ...
      send(data: string) {
        ...
        if (msg.method === 'SYNC') {
          ...
          this.respond(msg.id!, { history: [
            {
              eventId: 'm1',
              timestamp: Date.now(),
              type: 'AGENT_MESSAGE',
              data: { content: 'Hello! I am a mock agent. You are running in ?mock mode. Try asking me to "run ls", "show diff", or "trigger file change".' }
            }
          ]})
      ...
      private simulateFileTreeChange() {
        this.notify('server_event', { eventId: 't-file', type: 'THOUGHT', data: { content: 'Simulating a file system change (e.g. creating new_file.ts)...' } })
        ...
        setTimeout(() => {
          ...
          this.simulateTypewriterResponse('I have detected a file system change. The sidebar file tree has been updated automatically to show `new_file.ts`.')
        }, 1500)
      }

      private simulateTypewriterResponse(text: string) {
        this.notify('server_event', { eventId: 't-' + Date.now(), type: 'THOUGHT', data: { content: 'Processing request...' } })
        
        let current = 0
        const interval = setInterval(() => {
          if (current < text.length) {
            this.notify('server_event', { eventId: 'token-' + current + '-' + Date.now(), type: 'TOKEN', data: { content: text[current] } })
            current++
          } else {
      ...
      private simulateToolExecution(_command: string) {
        ...
        const interval = setInterval(() => {
          if (lineIdx < lines.length) {
            ...
            this.simulateTypewriterResponse('I have listed the files for you. You can click the "Logs" button in the tool card above to see the full output in the Inspector.')
          }
        }, 200)
        this.activeIntervals.push(interval)
      }
      ...
      notify(method: string, params: any) {
        ...
        setTimeout(() => this.onmessage && this.onmessage({ data: JSON.stringify(notification) }), 10)
      }
      close() {
        this.clearSimulations()
      }
