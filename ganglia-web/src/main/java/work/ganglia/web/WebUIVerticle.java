...
public class WebUIVerticle extends AbstractVerticle {
    ...
    private void broadcastFileTree() {
        buildFileTreeAsync(Path.of(".")).onSuccess(tree -> {
            sessionSockets.keySet().forEach(sessionId -> {
    ...
    private void setupWebSocket(ServerWebSocket ws) {
        ...
        ws.textMessageHandler(text -> {
            try {
                ...
                String sessionId = request.params() != null ? request.params().getString("sessionId") : null;
                if (sessionId != null) {
    ...
    private void setupEventBusConsumers() {
        ...
        vertx.eventBus().<JsonObject>consumer(Constants.ADDRESS_UI_OUTBOUND_CACHE, msg -> {
            String sessionId = msg.headers().get("sessionId");
            if (sessionId != null) {
                sessionHistories.computeIfAbsent(sessionId, k -> new java.util.ArrayList<>()).add(msg.body());
            }
        });

        // Forward internal events to WebSocket
        vertx.eventBus().<JsonObject>consumer("ganglia.ui.ws.events", msg ->
            forwardToWebSocket(msg.headers().get("sessionId"), "server_event", msg.body()));

        // Forward TTY streams to WebSocket
        vertx.eventBus().<JsonObject>consumer("ganglia.ui.ws.tty", msg ->
            forwardToWebSocket(msg.headers().get("sessionId"), "tty_event", msg.body()));
    }
    ...
    private void handleListFiles(JsonRpcRequest request, ServerWebSocket ws, String sessionId) {
        buildFileTreeAsync(Path.of(".")).onComplete(res -> {
            if (res.succeeded()) {
    ...
    private void handleStart(JsonRpcRequest request, ServerWebSocket ws, String sessionId) {
        ...
        ServerEvent userEvent = createServerEvent(EventType.USER_MESSAGE, new ServerEvent.UserMessageData(prompt));
    ...
    private void handleRetry(JsonRpcRequest request, ServerWebSocket ws, String sessionId) {
        ...
        if (lastPrompt != null) {
            sessionManager.getSession(sessionId).onComplete(res -> {
                if (res.succeeded() && res.result() != null) agentLoop.run(lastPrompt, res.result(), new AgentSignal());
            });
        }
        ...
        vertx.executeBlocking(() -> {
            try {
                if ("WORKSPACE_DIFF_VIRTUAL_PATH".equals(filePath)) {
                    ...
                    publishEvent(sessionId, EventType.FILE_CONTENT, new ServerEvent.FileContentData(filePath, mockDiff, "diff"));
                } else {
                    Path path = Path.of(filePath);
                    if (Files.exists(path) && !Files.isDirectory(path)) {
                        String content = Files.readString(path);
                        String ext = filePath.contains(".") ? filePath.substring(filePath.lastIndexOf('.') + 1) : "";
                        publishEvent(sessionId, EventType.FILE_CONTENT, new ServerEvent.FileContentData(filePath, content, ext));
                    }
                }
            } catch (Exception e) {
    ...
    private void publishEvent(String sessionId, EventType type, Object data) {
        forwardToWebSocket(sessionId, "server_event", JsonObject.mapFrom(createServerEvent(type, data)));
    }
    ...
    private Future<JsonObject> buildFileTreeAsync(Path root) {
        ...
        return fs.props(root.toString()).compose(props -> {
            if (props.isDirectory()) {
                node.put("type", "directory");
                return fs.readDir(root.toString()).compose(list -> {
                    List<Future<JsonObject>> childFutures = list.stream()
                        .map(Path::of)
                        .filter(p -> {
                            String fileName = p.getFileName().toString();
                            return (!fileName.startsWith(".") || fileName.equals(".ganglia")) && !fileName.equals("target");
                        })
                    ...
                    return Future.all(childFutures).map(cf -> {
