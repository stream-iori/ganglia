            ...
            message -> {
              testContext.verify(
                  () -> {
                    ...
                    assertEquals(expectedType.name(), event.getString("type"));
        ...
        .<JsonObject>consumer(
            ...
            message -> {
              testContext.verify(
                  () -> {
                    JsonObject event = message.body();
                    if (BaseEventType.TOKEN.name().equals(event.getString("type"))) {
                      assertEquals(token, event.getJsonObject("data").getString("content"));
                      testContext.completeNow();
                    }
                  });
            });
