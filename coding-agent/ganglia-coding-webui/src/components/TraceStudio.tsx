...
function getTypeColor(type: string) {
  ...
  if (type.includes('MANAGER') || type.includes('CYCLE'))
  ...
  if (type.includes('FACT'))
    return 'bg-teal-50 text-teal-700 border-teal-100 dark:bg-teal-950/30 dark:text-teal-400 dark:border-teal-900/50';
  if (type.includes('WORKTREE') || type.includes('MERGE'))
    return 'bg-pink-50 text-pink-700 border-pink-100 dark:bg-pink-950/30 dark:text-pink-400 dark:border-pink-900/50';
  if (type.includes('REALITY_ANCHOR'))
    return 'bg-lime-50 text-lime-700 border-lime-100 dark:bg-lime-950/30 dark:text-lime-400 dark:border-lime-900/50';
  ...
  if (type.includes('COLD_STORAGE'))
    return 'bg-stone-50 text-stone-700 border-stone-100 dark:bg-stone-950/30 dark:text-stone-400 dark:border-stone-900/50';
          ...
          <div className="flex-1 min-w-0">
            <div className="flex justify-between items-start mb-1.5 gap-4">
              <div className="flex flex-wrap items-center gap-2">
                ...
                {node.startEvent.type === 'MANAGER_GRAPH_CONVERGED' && (
                  <span className="text-[10px] font-bold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/20 px-2 py-0.5 rounded-full flex items-center gap-1 border border-emerald-100 dark:border-emerald-900/50">
                    <CheckCircle2 size={10} /> Converged
                  </span>
                )}

                {node.startEvent.type === 'MANAGER_GRAPH_STALLED' && (
                  <span className="text-[10px] font-bold text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20 px-2 py-0.5 rounded-full flex items-center gap-1 border border-red-100 dark:border-red-900/50">
                    <AlertCircle size={10} /> Stalled
                  </span>
                )}

                {node.startEvent.type === 'FACT_SUPERSEDED' && (
                ...
                {node.startEvent.type === 'FINGERPRINT_CACHE_HIT' && (
                  <span className="text-[10px] font-bold text-sky-600 dark:text-sky-400 bg-sky-50 dark:bg-sky-900/20 px-2 py-0.5 rounded-full flex items-center gap-1 border border-sky-100 dark:border-sky-900/50">
                    <CheckCircle2 size={10} /> Cached
                  </span>
                )}
            ...
            <div className="flex items-start gap-2">
              ...
              <div className={`text-sm font-sans font-medium break-words ${node.startEvent.type === 'FACT_SUPERSEDED' ? 'line-through text-slate-400 dark:text-slate-500' : 'text-slate-900 dark:text-slate-100'}`}>
