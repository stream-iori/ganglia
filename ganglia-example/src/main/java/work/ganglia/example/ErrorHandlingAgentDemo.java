 ...
 * asked to perform a task that starts with a failure and must autonomously decide how to recover or
 * report the error.
 */
public class ErrorHandlingAgentDemo {
              ...
              vertx
                  ...
                  .writeFile(
                      ...
                      io.vertx.core.buffer.Buffer.buffer(
                          "This is the actual data you were looking for."))
                  .onSuccess(
                      v -> {
                        ...
                        ganglia
                            ...
                            .compose(
                                context -> runInteractiveLoop(ganglia, sessionId, input, context))
                            .onComplete(
                                ar -> {
                                  if (ar.succeeded()) {
                                    System.out.println("\n\n--- Workflow Complete ---");
  ...
  private static Future<Void> runInteractiveLoop(
      Ganglia ganglia, String sessionId, String input, SessionContext context) {
    return ganglia
        .agentLoop()
        .run(input, context)
        .compose(
