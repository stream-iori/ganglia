...
To maintain focus, Sub-Agents use `ContextScoper` (in `work.ganglia.kernel.subagent`):
...
- **Mission Context**: If a `TaskNode` carries `missionContext`, it is propagated to child metadata and prepended to the child's prompt as `"MISSION: {m}\n\nTASK: {t}"`.
...
2. **`DefaultGraphExecutor`**: Infrastructure used when multiple sub-agents are orchestrated in a DAG. Supports mission context propagation and selective `inputMapping` wiring.
3. **`Blackboard`**: Port interface for cross-cycle fact sharing between graph nodes (`InMemoryBlackboard` adapter).
4. **`Fact` / `FactStatus`**: Immutable record representing a versioned finding with lifecycle tracking (ACTIVE → SUPERSEDED → ARCHIVED).
