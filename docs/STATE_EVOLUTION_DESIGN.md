...
All core state objects in Ganglia (e.g., `SessionContext`, `Turn`, `Message`, `Fact`, `TaskNode`) are implemented as Java 17 `record` types. We strictly follow the **Functional Update** pattern: **state, once created, is never mutated; any change produces a new snapshot**.
...
1. **Semantic clarity**: Only `Role.TOOL` messages carry tool observation data. Similarly, `Fact` records carry `FactStatus` (ACTIVE/SUPERSEDED/ARCHIVED) to track lifecycle.
2. **Type safety**: Eliminates the need for null checks or ambiguity from flattened fields.

The `Blackboard` fact store follows the same immutability principle: facts are never mutated in-place. Superseding a fact creates a new `Fact` record with an incremented `version` and `SUPERSEDED` status.
