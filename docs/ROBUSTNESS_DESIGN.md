...
### 2.5 Active Cancellation with AbortReason
...
`AgentSignal` now supports categorized abort reasons via the `AbortReason` enum:
- **USER_CANCELLED**: User pressed Ctrl+C or explicitly cancelled.
- **MISSION_SUPERSEDED**: A Blackboard fact invalidated the running mission.
- **BUDGET_EXCEEDED**: Token or cycle budget exhausted.
- **STALL_DETECTED**: No progress detected in N cycles.

This allows long-running tasks to distinguish between "discard everything" (user cancel) and "save intermediate results" (mission superseded).
...
### 5.3 Blackboard Consistency

The `Blackboard` uses append-only semantics with optimistic versioning to minimize contention:
- **Optimistic Concurrency**: Each `Fact` carries a monotonic `version`. Supersede operations use `ConcurrentHashMap.replace(key, expected, new)` — a version conflict results in a retry.
- **Stall Detection**: `getNewFactCount(lastNCycles)` enables the engine to detect when no progress is being made across cycles.
- **L1/L2 Tiered Storage**: Active fact summaries (L1) stay in context; full details (L2) are stored as cold references retrieved on-demand.
