...
- **Internal Ports:** Define how the Kernel interacts with its internal systems. Crucially includes `ExecutionContext` which allows tools to emit stream chunks without knowing about Vert.x. Also includes `Blackboard` — an append-only fact store for cross-cycle state sharing between Manager nodes.
- **External Ports:** Define interactions with LLMs (`ModelGateway`) and Environment (`ToolSet`).
- **Domain Models:** Records like `Message`, `SessionContext`, `Fact`, and `ObservationEvent` ensure data integrity.
...
2. **Hard Abort:** An `AgentSignal` provides **active cancellation** with categorized `AbortReason` (USER_CANCELLED, MISSION_SUPERSEDED, BUDGET_EXCEEDED, STALL_DETECTED). It allows for immediate termination of network calls (HTTP stream reset), tool executions, and token publishing, ensuring the UI stops instantly.
